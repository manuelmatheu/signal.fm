...
function md5(str) {
  ...
  var l = utf8.length;
  var numBlocks = Math.ceil((l + 9) / 64);
  var m = new Array(numBlocks * 16).fill(0);
  for (var i = 0; i < l; i++) { m[i >> 2] |= utf8.charCodeAt(i) << ((i % 4) * 8); }
  m[l >> 2] |= 0x80 << ((l % 4) * 8);
  m[numBlocks * 16 - 2] = l * 8;
  ...
  function hex(n) { var s=''; for (var j=0;j<4;j++) s+=('0'+((n>>(j*8+4))&0xf).toString(16)).slice(-1)+('0'+((n>>(j*8))&0xf).toString(16)).slice(-1); return s; }
